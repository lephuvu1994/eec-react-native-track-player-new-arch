...
import TrackPlayer from './NativeTPMusicModule';
...
const emitter = new NativeEventEmitter(
  (TrackPlayer as unknown as NativeModule) ?? undefined
);
...
export async function add(
  ...
  return resolvedTracks.length < 1
    ? undefined
    : TrackPlayer?.add(
        resolvedTracks as unknown as AddTrack[],
        insertBeforeIndex
      );
}
...
export async function updateOptions({
  ...
  return TrackPlayer?.updateOptions({
    ...
    previousIcon: resolveImportedAsset(options.previousIcon) as
      | number
      | undefined,
    ...
    forwardIcon: resolveImportedAsset(options.forwardIcon) as
      | number
      | undefined,
  });
}
...
export async function setRepeatMode(mode: RepeatMode): Promise<RepeatMode> {
  return (
    (TrackPlayer?.setRepeatMode(
      mode as unknown as number
    ) as unknown as RepeatMode) ?? Promise.resolve(RepeatMode.Off)
  );
}
...
export async function getActiveTrack(): Promise<Track | undefined> {
  return (
    ((await TrackPlayer?.getActiveTrack()) as unknown as Track) ?? undefined
  );
}
...
export async function getProgress(): Promise<Progress> {
  return (
    (TrackPlayer?.getProgress() as unknown as Progress) ?? ({} as Progress)
  );
}
...
export async function getState(): Promise<State> {
  return (
    (((await TrackPlayer?.getPlaybackState()) as PlaybackState)
      ?.state as unknown as State) ?? ({} as State)
  );
}
...
export async function getPlaybackState(): Promise<PlaybackState> {
  return (
    (TrackPlayer?.getPlaybackState() as unknown as PlaybackState) ??
    ({} as PlaybackState)
  );
}
...
export async function getRepeatMode(): Promise<RepeatMode> {
  return (
    (TrackPlayer?.getRepeatMode() as unknown as RepeatMode) ??
