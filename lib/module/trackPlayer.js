...
import TrackPlayer from "./NativeTPMusicModule.js";
import { RepeatMode } from "./constants/index.js";
import resolveAssetSource from './resolveAssetSource';
const emitter = new NativeEventEmitter(TrackPlayer ?? undefined);
...
export async function setRepeatMode(mode) {
  return TrackPlayer?.setRepeatMode(mode) ?? Promise.resolve(RepeatMode.Off);
}
...
export async function getQueue() {
  return TrackPlayer?.getQueue() ?? Promise.resolve([]);
}
...
export async function getRepeatMode() {
  return TrackPlayer?.getRepeatMode() ?? Promise.resolve(RepeatMode.Off);
}
